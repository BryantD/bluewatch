...
def get_last_read_timestamp(db_path: str, scan_name: str) -> str | None:
    ...
    conn = sqlite3.connect(db_path)
    cursor = conn.execute(
        "SELECT last_read_timestamp FROM scan_state WHERE scan_name = ?", (scan_name,)
    )
...
@click.option(
    ...
    type=int,
    default=24,
    help="Hours to look back for posts (default: 24)",
)
def scan(scan_name, config, log_level, lookback_hours):
    ...
    try:
        ...
        raise click.ClickException(
            "The atproto library is required. Install via `uv add atproto`."
        )
...
def status(scan_name, config, log_level):
    ...
    for row in results:
        ...
        print(
            f"{scan_name:<20} {handle:<20} {last_read_short:<20} {last_run_short:<20}"
        )
...
@click.option(
    "--execute", is_flag=True, help="Execute webhook/shell commands if pattern matches"
)
def test(scan_name, post_url, config, log_level, execute):
    ...
    if len(path_parts) < 4 or path_parts[0] != "profile" or path_parts[2] != "post":
        logger.error(
            "Invalid post URL format. Expected: https://bsky.app/profile/HANDLE/post/POST_ID"
        )
            ...
            if execute:
                ...
                if shell_cmd:
                    try:
                        ...
                        escaped_data = {
                            k: shlex.quote(str(v)) for k, v in match_data.items()
                        }
...
def fetch_posts_backwards(client, handle, last_read_timestamp, max_age_hours=24):
    ...
    while True:
        try:
            ...
            if api_calls > 1:
                logger.debug("Pausing 10 seconds between API calls...")
                time.sleep(10)
    ...
    if matches_found:
        ...
        if webhook_url:
            try:
                ...
                response.raise_for_status()
                logger.info(
                    f"Webhook called successfully for {name}: {response.status_code}"
                )
            except requests.RequestException as e:
            ...
            for match in matches_found:
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        logger.error(
                            f"Shell command failed for {name}: {result.stderr}"
                        )
                except Exception as e:
